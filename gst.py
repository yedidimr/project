...
"""
Generalized Suffix-Tree - find longest 

this code is copied from https://github.com/ptrus/suffix-trees/blob/master/suffix_trees/STree.py
with a few changes yedidimr added:
    1 -option to control the alphabet (letters) and
    2 - if the longest common substring is of length x, than return all common substrings of length x

if you want to use the following strings
all_data = ["xxxabcxxx", "adsaabc", "ytysabcrew", "qqqabcqw", "aaabc"]
first convert each string to a list of letters:
data = [['x', 'x', 'x', 'a', 'b', 'c', 'x', 'x', 'x'], ['a', 'd', 's', 'a', 'a', 'b', 'c'], ['y', 't', 'y', 's', 'a', 'b', 'c', 'r', 'e', 'w'], ['q', 'q', 'q', 'a', 'b', 'c', 'q', 'w'], ['a', 'a', 'a', 'b', 'c']]
(you can do this by running the line:
    data = [list(d) for d in all_data])

than run the suffix tree:

st = STree(data)
lcs_list , lcs_set = st.lcs()


why is that good? you can choose you letters. it doesnt have to be the abc but amino acids etc
for example:
you might have the protein with the amino acids:
a = ['A.509', 'A.512', 'A.512', 'A.514', 'A.515']
and another protein with amino acids
b =['A.514', 'A.515', 'A.516', 'A.517', 'A.523', 'A.524', 'A.527', 'A.527']
each protein is a string and each aa is a letter.
now you can search the longest amino acids common sequence in the protein by running
st = STree([a,b])
lcs_list , lcs_set = st.lcs()
...
class STree():
    ...
    def _check_input(self, input):
        ...
        raise ValueError("String argument should be of type list or"
                                     " a list of lists")
    ...
    def _build_generalized(self, xs):
        ...
        for i in range(len(xs)):
            one_word.extend(xs[i])
            one_word.append(next(terminal_gen))

        self.word = one_word
        self._generalized_word_starts(xs)
        # import pdb
        # pdb.set_trace()
        self._build(one_word)
        # pdb.set_trace()
    ...
    def lcs(self, stringIdxs=-1):
        ...
        all_lcs = []
        for deepestNode in deepestNodes:
            start = deepestNode.idx
            end = deepestNode.idx + deepestNode.depth
            all_lcs.append(self.word[start:end])
        return all_lcs, set([" ".join(l) for l in all_lcs])
    ...
    def _find_lcs(self, node, stringIdxs):
        ...
        for (n,_) in node.transition_links:
            ...
            if n.generalized_idxs.issuperset(stringIdxs):
                ...
                import pdb
                # pdb.set_trace()                    
                nodes.extend(self._find_lcs(n, stringIdxs))
        ...
        import pdb
        try:
            a_max_node = max(nodes, key=lambda n: n.depth)
        except:
            pdb.set_trace()
        maxDepth = a_max_node.depth
        deepestNodes = [node for node in nodes if node.depth == maxDepth]
        return deepestNodes
